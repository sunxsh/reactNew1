...
export const expandIcon = ({ expanded, onExpand, record },flag) =>{
  ...
  return (
   ...
   <MinusCircleTwoTone onClick={e => onExpand(record, e)} style={style}/>
  ) : (
   <PlusCircleTwoTone onClick={e => onExpand(record, e)} style={style} />
  ) : null)
}
 ...
 * @param {Blob} res | Blob文件流对象
 * @param {String} fileName | 导出文件的名字
 */
export const exportFileBlob = (res,fileName)=>{
  const tools = new Tools();
  const blob = new Blob([res], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  const url = window.URL.createObjectURL(blob);
  tools.createALabel(url,fileName);
}
