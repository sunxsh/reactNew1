...
const LayoutTable = memo(function LayoutTable({ btnGrounp,
  ...
  const columns = useRef(tableProps.columns);

  const elemet = useRef(null);

  useEffect(() => {
    forceUpdate();
    columns.current = tableProps.columns;
  }, [tableProps.columns])
  ...
  return (
    <ConfigProvider getPopupContainer={() => (elemet.current || document.body)}>
      <div className={style.layoutTable} ref={elemet}>
        ...
        <Table loading={loading} size={tableSize} {...tableProps} columns={columns.current} />
